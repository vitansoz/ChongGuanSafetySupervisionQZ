...
{
    public class FingerBitmapFormat
    {
        ...
        public static void GetBitmap(byte[] buffer, int nWidth, int nHeight, ref MemoryStream ms)
        {
            ...
            try
            {
                FingerBitmapFormat.BITMAPFILEHEADER bitmapfileheader = new FingerBitmapFormat.BITMAPFILEHEADER();
                FingerBitmapFormat.BITMAPINFOHEADER bitmapinfoheader = new FingerBitmapFormat.BITMAPINFOHEADER();
                FingerBitmapFormat.MASK[] maskArray = new FingerBitmapFormat.MASK[length];
                ...
                ms.Write(FingerBitmapFormat.StructToBytes((object)bitmapfileheader, 14), 0, 14);
                ms.Write(FingerBitmapFormat.StructToBytes((object)bitmapinfoheader, Marshal.SizeOf((object)bitmapinfoheader)), 0, Marshal.SizeOf((object)bitmapinfoheader));
                for (int index = 0; index < length; ++index)
                {
                    ...
                    ms.Write(FingerBitmapFormat.StructToBytes((object)maskArray[index], Marshal.SizeOf((object)maskArray[index])), 0, Marshal.SizeOf((object)maskArray[index]));
                }
                FingerBitmapFormat.RotatePic(buffer, nWidth, nHeight, ref ResBuf);
        ...
        public static void WriteBitmap(byte[] buffer, int nWidth, int nHeight)
        {
            ...
            try
            {
                ...
                binaryWriter.Write(FingerBitmapFormat.StructToBytes((object)bitmapfileheader, 14));
                binaryWriter.Write(FingerBitmapFormat.StructToBytes((object)bitmapinfoheader, Marshal.SizeOf((object)bitmapinfoheader)));
                for (int index = 0; index < length; ++index)
                {
                    maskArray[index].redmask = (byte)index;
                    maskArray[index].greenmask = (byte)index;
                    maskArray[index].bluemask = (byte)index;
                    maskArray[index].rgbReserved = (byte)0;
                    binaryWriter.Write(FingerBitmapFormat.StructToBytes((object)maskArray[index], Marshal.SizeOf((object)maskArray[index])));
                }
                FingerBitmapFormat.RotatePic(buffer, nWidth, nHeight, ref ResBuf);
                byte[] numArray = (byte[])null;
                if (num2 - nWidth > 0)
